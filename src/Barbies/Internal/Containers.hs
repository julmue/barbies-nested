...
instance FunctorB (b :: (Type -> Type) -> Type) => Functor (Container b) where
  fmap f = Container . bmap (first f) . getContainer

instance TraversableB (b :: (Type -> Type) -> Type) => Foldable (Container b) where
  foldMap f = bfoldMap (f . getConst) . getContainer

instance TraversableB (b :: (Type -> Type) -> Type) => Traversable (Container b) where
    traverse f = fmap Container . btraverse (bitraverse f pure) . getContainer

instance ApplicativeB (b :: (Type -> Type) -> Type) => Applicative (Container b) where
    pure a
...
instance FunctorB (b :: (Type -> Type) -> Type) => Functor (ErrorContainer b) where
  fmap f = ErrorContainer . (bmap (first f)) . getErrorContainer

instance TraversableB (b :: (Type -> Type) -> Type) => Foldable (ErrorContainer b) where
  foldMap f = bfoldMap (either f (const mempty)) . getErrorContainer

instance TraversableB (b :: (Type -> Type) -> Type) => Traversable (ErrorContainer b) where
    traverse f = fmap ErrorContainer . btraverse (bitraverse f pure) . getErrorContainer
